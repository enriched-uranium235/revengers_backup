...
from .forms import InquiryForm, DiaryCreateForm, NoticeCreateForm, CommentCreateForm, DMForm, NewCommentForm
from .models import Avengers, Comment, Notice, Post, Good, Good2, Good3, Good4, Favorite, Report, Report2, Report3, Report4, Report5, Report1_1, Report2_1, Report3_1, Report4_1, Report5_1, BlackList, AllBlackList, AvengersLog, DM, AttendanceRecord, AttendanceAllRecord, Comment2
...
# 選択した相手にDMを送る
def dm(request, id):
    select = get_object_or_404(Follow, user=id)
...
def good(request, good_id):
    ...
    # ゼロより大きければ既にgood済み
    if is_good > 0:
        messages.info(request, 'すでにこのメッセージにいいね!をしています。')
        return redirect('avengers:community_room')
    ...
    good.save()
    # メッセージを設定
    messages.success(request, 'この投稿にいいね!しました！')
    return redirect('avengers:community_room')
...
def good2(request, good_id):
    ...
    # ゼロより大きければ既にgood済み
    if is_good > 0:
        messages.info(request, 'すでにこの投稿にいいね!をしています。')
        return redirect('avengers:accuse_list')
...
def good3(request, good_id):
    ...
    if is_good > 0:
        messages.info(request, 'すでにこの投稿にいいね!をしています。')
        return redirect('avengers:accuse_detail', pk=good_id)
...
def good4(request, pk, good_id):
    ...
    good.save()
    # メッセージを設定
    messages.success(request, 'このコメントにいいね!しました！')
    return redirect('avengers:accuse_detail', pk=post.pk)

# goodボタンの処理(Reply用)
def good5(request, pk, good_id):
    post = get_object_or_404(Avengers, pk=pk)
    # goodするReplyを取得
    good_msg = Comment.objects.get(id=good_id)
    # 自分がReplyにGoodした数を調べる
    is_good = Comment.objects.filter(user=request.user) \
        .filter(comment=good_msg).count()
    # ゼロより大きければ既にgood済み
    if is_good > 0:
...
def good6(request, pk, good_id):
    post = get_object_or_404(Post, pk=pk)
    # goodするCommentを取得
    good_msg = Comment2.objects.get(id=good_id)
    # 自分がCommentにGoodした数を調べる
    is_good = Comment2.objects.filter(author=request.user) \
        .filter(content=good_msg).count()
    # ゼロより大きければ既にgood済み
    if is_good > 0:
        messages.info(request, 'すでにこのコメントにいいね!をしています。')
        return redirect('avengers:post-detail', pk=post.pk)
...
def add_avengers_log(request, pk):
    ...
    log.group = post.group
    log.content = post.content
    log.photo1 = post.photo1
    log.media1 = post.media1
    log.media2 = post.media2
    log.number = post.pk
    log.created_at = post.created_at
    log.save()
    return redirect('avengers:accuse_detail', pk=post.pk)
...
def blacksubscribe1(request, subscribe_id):
    ...
    r.save()
    # メッセージを設定
    messages.info(request,'この投稿を通報しました。通報いただきありがとうございます。運営側で確認をしたうえで対処させていただきます。')
    return redirect('avengers:accuse_detail', pk=subscribe_id)
...
def blacksubscribe2(request, pk, comment_pk):
    ...
    r.save()
    # メッセージを設定
    messages.info(request,'このコメントを通報しました。通報いただきありがとうございます。運営側で確認をしたうえで対処させていただきます。')
    return redirect('avengers:accuse_detail', pk=post.pk)
...
def blacksubscribe3(request, pk):
    ...
    r.save()
    # メッセージを設定
    messages.info(request,'このスレッドを通報しました。通報いただきありがとうございます。運営側で確認をしたうえで対処させていただきます。')
    return redirect('avengers:community_room')
...
def blacksubscribe4(request, pk):
    ...
    r.save()
    # メッセージを設定
    messages.info(request,'このDMを通報しました。通報いただきありがとうございます。運営側で確認をしたうえで対処させていただきます。')
    return redirect('avengers:dm_list')
...
def excel_export(request):
    ...
    sheet = book.add_worksheet('Avengersレポート履歴')
    ...
    response['Content-Disposition'] = "attachment; filename=avengers_all_report.xlsx"
...
class SearchLawyer(generic.TemplateView):
    template_name = 'search_lawyer.html'

#都道府県情報から心療内科を探す
class SearchPsycho(generic.TemplateView):
    template_name = 'psycho_medicine.html'
...
# 心療内科検索(ログイン前,PC)
class UserManualPCPsychosomaticMedicine(generic.TemplateView):
    template_name = 'manual/user_manual_pc_psychosomaticmedicine_before_login.html'
# サインアップ(PC)
...
# 心療内科検索(ログイン後,PC)
class UserManualPCPsychosomaticMedicine2(generic.TemplateView):
    template_name = 'manual/user_manual_pc_psychosomaticmedicine_after_login.html'
# MyPage(画面説明,PC)
class UserManualPCMyPage(generic.TemplateView):
    template_name = 'manual/user_manual_pc_mypage.html'
# MyPage プロフィール編集(PC)
class UserManualPCMyPageChangeProfile(generic.TemplateView):
    template_name = 'manual/user_manual_pc_mypage_changeprofile.html'
# MyPage フォローリスト(PC)
class UserManualPCMyPageFollowCheck(generic.TemplateView):
    template_name = 'manual/user_manual_pc_mypage_followcheck.html'
# MyPage フォロワーリスト(PC)
class UserManualPCMyPageFollowerCheck(generic.TemplateView):
    template_name = 'manual/user_manual_pc_mypage_followercheck.html'
# MyPage 表示履歴(PC)
...
# MyPage 一覧に戻る(PC)
class UserManualPCMyPageReturn(generic.TemplateView):
    template_name = 'manual/user_manual_pc_mypage_detail_return.html'
# MyPage 投稿削除(PC)
...
# Everyone'sPost 詳細から戻る(PC)
class UserManualPCEveryonesPostDetailReturn(generic.TemplateView):
    template_name = 'manual/user_manual_pc_everyonespost_detail_return.html'
...
# CommunityRoom DM受信プロフィール編集(PC)
class UserManualPCCommunityRoomDMSentChangeProfile(generic.TemplateView):
    template_name = 'manual/user_manual_pc_communityroom_dm_sent_changeprofile.html'
# CommunityRoom DM受信フォローリスト(PC)
class UserManualPCCommunityRoomDMSentFollowCheck(generic.TemplateView):
    template_name = 'manual/user_manual_pc_communityroom_dm_sent_followcheck.html'
# CommunityRoom DM受信フォロワーリスト(PC)
class UserManualPCCommunityRoomDMSentFollowerCheck(generic.TemplateView):
    template_name = 'manual/user_manual_pc_communityroom_dm_sent_followercheck.html'
...
# CommunityRoom DM送信プロフィール変更(PC)
class UserManualPCCommunityRoomDMSendChangeProfile(generic.TemplateView):
    template_name = 'manual/user_manual_pc_communityroom_dm_send_changeprofile.html'
# CommunityRoom DM送信フォローリスト(PC)
class UserManualPCCommunityRoomDMSendFollowCheck(generic.TemplateView):
    template_name = 'manual/user_manual_pc_communityroom_dm_send_followcheck.html'
# CommunityRoom DM送信フォロワーリスト(PC)
class UserManualPCCommunityRoomDMSendFollowerCheck(generic.TemplateView):
    template_name = 'manual/user_manual_pc_communityroom_dm_send_followercheck.html'
...
# CommunityRoom DM送信詳細戻る(PC)
class UserManualPCCommunityRoomDMSendDetailReturn(generic.TemplateView):
    template_name = 'manual/user_manual_pc_communityroom_dm_send_detail_return.html'
...
# 心療内科検索(ログイン前,Tablet)
class UserManualTabletPsychosomaticMedicine(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_psychosomaticmedicine_before_login.html'
# サインアップ(Tablet)
...
# 心療内科検索(ログイン後,Tablet)
class UserManualTabletPsychosomaticMedicine2(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_psychosomaticmedicine_after_login.html'
# MyPage(画面説明,Tablet)
class UserManualTabletMyPage(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_mypage.html'
# MyPage プロフィール編集(Tablet)
class UserManualTabletMyPageChangeProfile(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_mypage_changeprofile.html'
# MyPage フォローリスト(Tablet)
class UserManualTabletMyPageFollowCheck(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_mypage_followcheck.html'
# MyPage フォロワーリスト(Tablet)
class UserManualTabletMyPageFollowerCheck(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_mypage_followercheck.html'
# MyPage 表示履歴(Tablet)
...
# MyPage 一覧に戻る(Tablet)
class UserManualTabletMyPageReturn(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_mypage_detail_return.html'
# MyPage 投稿削除(Tablet)
...
# Everyone'sPost 詳細から戻る(Tablet)
class UserManualTabletEveryonesPostDetailReturn(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_everyonespost_detail_return.html'
...
# CommunityRoom DM受信プロフィール編集(Tablet)
class UserManualTabletCommunityRoomDMSentChangeProfile(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_communityroom_dm_sent_changeprofile.html'
# CommunityRoom DM受信フォローリスト(Tablet)
class UserManualTabletCommunityRoomDMSentFollowCheck(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_communityroom_dm_sent_followcheck.html'
# CommunityRoom DM受信フォロワーリスト(Tablet)
class UserManualTabletCommunityRoomDMSentFollowerCheck(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_communityroom_dm_sent_followercheck.html'
...
# CommunityRoom DM送信プロフィール変更(Tablet)
class UserManualTabletCommunityRoomDMSendChangeProfile(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_communityroom_dm_send_changeprofile.html'
# CommunityRoom DM送信フォローリスト(Tablet)
class UserManualTabletCommunityRoomDMSendFollowCheck(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_communityroom_dm_send_followcheck.html'
# CommunityRoom DM送信フォロワーリスト(Tablet)
class UserManualTabletCommunityRoomDMSendFollowerCheck(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_communityroom_dm_send_followercheck.html'
...
# CommunityRoom DM送信詳細戻る(Tablet)
class UserManualTabletCommunityRoomDMSendDetailReturn(generic.TemplateView):
    template_name = 'manual/user_manual_tablet_communityroom_dm_send_detail_return.html'
...
# 心療内科検索(ログイン前,SP)
class UserManualSPPsychosomaticMedicine(generic.TemplateView):
    template_name = 'manual/user_manual_smartphone_psychosomaticmedicine_before_login.html'
# サインアップ(SP)
...
# 心療内科検索(ログイン後,SP)
class UserManualSPPsychosomaticMedicine2(generic.TemplateView):
    template_name = 'manual/user_manual_smartphone_psychosomaticmedicine_after_login.html'
# MyPage(画面説明,SP)
...
# MyPage 一覧に戻る(SP)
class UserManualSPMyPageReturn(generic.TemplateView):
    template_name = 'manual/user_manual_smartphone_mypage_detail_return.html'
# MyPage 投稿削除(SP)
...
# Everyone'sPost 詳細から戻る(SP)
class UserManualSPEveryonesPostDetailReturn(generic.TemplateView):
    template_name = 'manual/user_manual_smartphone_everyonespost_detail_return.html'
...
# CommunityRoom DM送信詳細戻る(SP)
class UserManualSPCommunityRoomDMSendDetailReturn(generic.TemplateView):
    template_name = 'manual/user_manual_smartphone_communityroom_dm_send_detail_return.html'
